...
export default function SharedGroupDetailScreen() {
  ...
  const { shareGroups, setShareGroups, userName } = useSettingsStore();
  ...
  const handleShareCode = async () => {
    ...
    try {
      await Share.share({
        message: `Join my "${group.name}" group on SubTrack! Use code: ${group.code}`,
              ...
              {memberCount === 0 ? (
                ...
                <View>
                  {group.members.map((m, i) => {
                    // Never hardcode labels like "Me" — show the current
                    // personalization name for this device's own row.
                    const displayName = m.me && userName.trim() ? userName.trim() : m.userName;
                    return (
                      <View key={m.userId}>
                        {i > 0 && <View style={styles.divider} />}
                        <View style={styles.memberRow}>
                          <Avatar size="small" name={displayName || "?"} bgColor={avatarColor(m.userId)} />
                          ...
                          ) : group.role === "owner" && !m.me ? (
                            <TouchableOpacity
                              onPress={() => handleRemoveMember(m.userId, displayName)}
                              activeOpacity={0.7}
