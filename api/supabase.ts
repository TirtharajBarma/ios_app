...
async function ensureSession(): Promise<SupabaseClient | null> {
  ...
  // Reset the cached promise on failure so a later call can retry instead of
  // being stuck with a dead session for the life of the process.
  if (!ok) sessionLoadPromise = null;
  return ok ? supabase : null;
}

/**
 * Current anonymous user id (used to identify "me" in a group's member list).
 * Returns null when Supabase is not configured. Throws when a session cannot
 * be established on a configured backend (so callers can keep local caches).
 */
export async function getCurrentUserId(): Promise<string | null> {
  const supabase = await ensureSession();
  if (!supabase) {
    if (!isSupabaseConfigured()) return null;
    throw new Error("Unable to reach sync server");
  }
...
/**
 * Push the user's display name to every group they belong to, so members see
 * the latest name instead of a stale create/join-time snapshot.
 */
export async function updateMyName(userName: string): Promise<void> {
  const supabase = await ensureSession();
  if (!supabase) return;
  const name = userName.trim();
  if (!name) return;
  const { error } = await supabase.rpc("update_my_name", { p_user_name: name });
  if (error) console.warn("Supabase: update name failed", error.message);
}
...
export async function fetchShareGroups(): Promise<GroupInfo[]> {
  const supabase = await ensureSession();
  if (!supabase) {
    // Un-configured backends (feature off) return empty; a configured backend
    // that can't be reached THROWS so callers keep their cached data.
...
/**
 * Push/update one shared subscription. Returns false when the remote call
 * failed (e.g. offline) so the caller can queue a retry.
 */
export async function upsertSharedSubscription(payload: UpsertPayload): Promise<boolean> {
  const supabase = await ensureSession();
...
/**
 * Delete one shared subscription. Returns false when the remote call failed
 * (e.g. offline) so the caller can queue a retry (tombstone).
 */
export async function deleteSharedSubscriptionRemote(id: string, groupId: string): Promise<boolean> {
  const supabase = await ensureSession();
  if (!supabase) return false;
  ...
  if (error) {
    console.warn("Supabase: delete shared sub failed", error.message);
    return false;
  }
  return true;
}
...
/**
 * Subscribe to real-time changes on shared_subscriptions table via Supabase Realtime channel.
 * Returns an unsubscribe callback function.
 */
export function subscribeToSharedChanges(onChange: () => void): () => void {
  const supabase = getClient();
  if (!supabase) return () => {};

  const channel = supabase
    .channel("shared_subscriptions_realtime")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "shared_subscriptions" },
      () => {
        onChange();
      }
    )
